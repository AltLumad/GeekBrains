...
class IParser(object):
    ...
    def _split_salary(self, salary):
        try:
            tmp = [c for c in salary.children]
            return [tmp[0].get_text(), tmp[2].get_text()]
        except:
            if 'до' in salary:
                salary = salary.get_text().split('до')
            else:
